...
if(move_uploaded_file($_FILES["file4"]['tmp_name'], $file_path)) {
 echo "The file ".basename( $_FILE["file4"]['name']);
} else{
 echo "Server File = ".$_FILES["file4"]['tmp_name'].", Temp name = ".$file_path;
}
 
 if($_POST["new"]=="yes")
{
 
$file_path = $target_path . basename( $_FILES["file5"]['name']);
